...
class PostUpdateViewTestCase(TestCase):
	def setUp(self):
		self.board = Board.objects.create(name='Django', description='Django des')
		self.username='kooki'
		self.password='123'
		user = User.objects.create_user(username=self.username,email='kooki@gmail.com',password=self.password)
		self.topic = Topic.objects.create(subject='Hello World' ,board=self.board, starter=user)
		self.post = Post.objects.create(message='This is mess',topic=self.topic, created_by=user)
